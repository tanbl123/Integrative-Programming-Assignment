            ...
            <?php if ($currentUser !== null): ?>
                <?php if ($currentUser->isReporter() || $currentUser->isAdmin()): ?>
                    <a
                        ...
                        title="Notifications"
                        aria-label="Notifications<?= $unreadNotifications > 0 ? ', ' . (int) $unreadNotifications . ' unread' : '' ?>"
                    >
                        <svg
                            viewBox="0 0 24 24"
                            width="18"
                ...
                <a class="button nav-button" href="<?= url('user/profile') ?>">
                    <?= e($currentUser->getFullName()) ?> · <?= e($currentUser->getRole()) ?>
                </a>
...
<main class="wrap">
    <?php if ($successMessage !== null): ?>
        <div class="alert alert-success" data-toast role="status">
            <?= e($successMessage) ?>
        </div>
    <?php endif; ?>

    <?php if ($errorMessage !== null): ?>
        <div class="alert alert-error" data-toast role="alert">
            <?= e($errorMessage) ?>
        </div>
    <?php endif; ?>