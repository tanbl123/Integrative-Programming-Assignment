...
try {
    ...
    foreach ($role in @('Reporter','Cleaner')) {
        $data = @{full_name="$prefix $role";email="$prefix.$($role.ToLower())@example.test";phone_no='0123456789';role=$role;account_status='Active';password=$password;password_confirmation=$password;ic_no='010101-01-1234';gender='Other';birth_date='2001-01-01';address_line1='Synthetic test address'}
        ...
        $created.Add(@{Id=[int]$entity.id; Email=$data.email; Role=$role})
    }
    ...
    $response = Request "user-api/$($reporter.Id)" 'PATCH' $admin.Session @{city='Test City';birth_date='2000-02-29';gender='Female'} $admin.Token
    ...
    $response = Request "user-api/$($cleaner.Id)" 'PUT' $admin.Session @{full_name="$prefix Updated cleaner";email=$cleaner.Email;phone_no='0123456789';role='Cleaner';account_status='Active';nationality='Test nationality'} $admin.Token
    ...
    $own = SignIn $reporter.Email $password
    ...
    $profile = Invoke-WebRequest "$BaseUrl/user/update-profile" -Method Post -WebSession $own.Session -Body @{_token=$own.Token;full_name="$prefix Own profile";email=$reporter.Email;phone_no='0123456789';city='Own City';birth_date='2002-02-02';role='Administrator';account_status='Active'} -SkipHttpErrorCheck
    ...
    foreach ($entity in $created) {
        ...
        $login = SignIn $entity.Email $password
